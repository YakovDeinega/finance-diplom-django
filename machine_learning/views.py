...
class MLPredictTickerAPIView(APIView):
    ...
    @extend_schema(
        ...
        summary='Предсказать цены закрытия свечей за определенный период',
        description=(
            f'Возвращает предсказанные цены закрытия свечей по одной указанной акции за определенный период, '
